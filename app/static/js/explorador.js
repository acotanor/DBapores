(() => {
  ...
  function renderPayload(payload) {
    ...
    // Render Featured (Promoted) section
    if (featured.length > 0 && promotedSection && promotedList) {
      promotedSection.classList.remove("d-none");
      promotedList.innerHTML = featured.map((game) => {
        const matchingTags = Array.isArray(game.tagsCoincidentes) ? game.tagsCoincidentes : [];
        const reason = matchingTags.length ? `Similar en: ${escapeHtml(matchingTags.join(", "))}` : "";
  ...
  form.addEventListener('submit', async (event) => {
    ...
    // Check if the input matches a name in our last search results to get the ID
    const matchingGame = lastSearchResults.find(g => g.name === appId);
    if (matchingGame) {
      appId = matchingGame.id;
      appIdInput.value = appId; // Update the input field to show the ID as requested
    }
